...
# API URL'si
api_url = "https://localhost:44317/api/Borsa/JaponYeni?daysAgo=500"
...
with open('datajpy.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['date', 'jpy'])  

    # Her bir <items> öğesini işle
    for item in soup.find_all('items'):
        date = item.find('date').text
        jpy = item.find('jpy').text
...
print("Veriler datajpy.csv dosyasına yazıldı.")
