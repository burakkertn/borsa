...
# API URL'si
api_url = "https://localhost:44317/api/Borsa/Euro?daysAgo=500"
...
with open('dataeur.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['date', 'eur'])  

    # Her bir <items> öğesini işle
    for item in soup.find_all('items'):
        date = item.find('date').text
        eur = item.find('eur').text
...
print("Veriler dataeur.csv dosyasına yazıldı.")
