...
gelecekteki_che_tahmini = polinommodel.predict(Xgelecek)

# MongoDB'ye bağlanın
client = pymongo.MongoClient("mongodb://localhost:27017/")  
database = client["borsadb"]  
collection = database["che"] 

# Geçmiş veriler
veri_dict = veri.to_dict(orient="records")  

collection.insert_many(veri_dict)

# Gelecekte tahminler
for tarih, tahmin in zip(gelecekteki_tarihler_sirali, gelecekteki_che_tahmini):
    tahmin_verisi = {
        "date": tarih,
        "che": tahmin[0]
    }
